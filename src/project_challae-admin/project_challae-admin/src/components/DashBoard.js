...
const MemberCount = () => {
    ...
    useEffect(() => {
        const memberCount = async () => {
            try {
                const response = await axios.get('http://localhost/api/memberCount');
                let data;
...
const ClubCount = () => {
    ...
    useEffect(() => {
        const clubCount = async () => {
            try {
                const response = await axios.get('http://localhost/api/clubCount');
                let data;
                if (typeof response.data === 'string') {
                    data = JSON.parse(response.data);
                } else {
...
const ArticleCount = () => {
    ...
    useEffect(() => {
        const articleCount = async () => {
            try {
                const response = await axios.get('http://localhost/api/articleCount');
                let data;
                if (typeof response.data === 'string') {
                    data = JSON.parse(response.data);
                } else {
